  ...
  <div class="space-y-6">
   <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-5 gap-4">
    ...
    <button @click="goToCreate"
        class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/30 transition-all active:scale-95 flex items-center shrink-0">
    ...
    <!-- Filters Area -->
   <div
      class="bg-white p-5 rounded-2xl shadow-lg shadow-gray-200/30 border border-gray-100 flex flex-wrap gap-4 items-end">
    ...
    <button @click="resetFilters"
        class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 font-semibold rounded-lg transition-colors text-sm">
        ری سیٹ
      </button>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/40 border border-gray-100 overflow-hidden">
          ...
          <tbody class="divide-y divide-gray-100">
           <!-- Loader Row -->
            ...
            <tr v-for="record in records.data" :key="record.id" class="hover:bg-gray-50/50 transition-colors group">
              <td class="py-4 px-6">
               <span class="px-3 py-1 bg-indigo-50 text-indigo-700 rounded-md text-sm font-semibold inline-block"
                  dir="ltr">{{ record.date }}</span>
              </td>
              <td class="py-4 px-6">
             ...
             <td
                class="py-4 px-6 text-gray-600 text-sm leading-relaxed max-w-sm truncate whitespace-normal line-clamp-2">
                {{ record.detail }}
              </td>
              <td class="py-4 px-6 text-center">
               <div
                  class="flex items-center justify-center space-x-2 space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity">
                  <button @click="editRecord(record.id)"
                    class="p-2 text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 rounded-lg transition-colors"
                    title="ترمیم">
           ...
           <tr v-if="!loading && (!records.data || records.data.length === 0)">
      ...
      <!-- Pagination -->
     <div v-if="records.total > records.per_page"
        ...
        <div class="flex space-x-2 space-x-reverse">
         <button :disabled="!records.prev_page_url" @click="changePage(records.current_page - 1)"
            class="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium hover:bg-white disabled:opacity-50 disabled:bg-transparent transition-colors">
            پچھلا
          </button>
...
export default {
  ...
  setup() {
    ...
    watch(selectedStudent, (val, oldVal) => {
      if (val?.id === oldVal?.id) return

      filters.value.student_id = val ? val.id : ''
      fetchRecords()
    })
