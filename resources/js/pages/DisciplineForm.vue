  ...
  <div class="space-y-6 max-w-3xl mx-auto">
    <div class="mb-8 border-b border-gray-200 pb-5 flex items-center justify-between">
      <div>
       ...
       <p class="text-sm text-gray-500 mt-4 font-medium">طالب علم کی حاضری یا ڈسپلن کی کارروائی درج کریں</p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-2 md:col-span-2">
           <label class="block text-sm font-semibold text-gray-700">طالب علم <span
          ...
          <div class="space-y-2">
           <label class="block text-sm font-semibold text-gray-700">درجہ / کلاس <span
                class="text-red-500">*</span></label>
            <input v-model="form.class" type="text" required readonly
          ...
          <div class="space-y-2">
           <label class="block text-sm font-semibold text-gray-700">نوعیت (Type) <span
                class="text-red-500">*</span></label>
          ...
          <div class="space-y-2 md:col-span-2">
           ...
           <input v-model="form.date" type="date" required dir="ltr"
              class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-colors text-right" />
          </div>

          <div class="space-y-2 md:col-span-2">
            <label class="block text-sm font-semibold text-gray-700">تفصیلات</label>
           <textarea v-model="form.detail" rows="4"
              class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 outline-none transition-colors resize-none"
              placeholder="مزید تفصیلات یہاں درج کریں..."></textarea>
          </div>
        </div>

      <div v-if="error"
          class="p-4 bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl font-medium flex items-center">
        ...
        <div class="flex justify-end pt-6 border-t border-gray-100 space-x-3 space-x-reverse">
         <button type="button" @click="goBack"
            class="px-6 py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold transition-colors">منسوخ
            کریں</button>
          <button type="submit" :disabled="loading"
            class="px-8 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/30 transition-all disabled:opacity-50 flex items-center">
...
export default {
  ...
  setup() {
    ...
    const setSelectedStudent = () => {
      if (!form.value.student_id || students.value.length === 0) return

      const student = students.value.find(
        s => s.id === form.value.student_id
      )
    ...
    const customLabel = (option) => {
      return `${option.sname} (${option.code})`
    }
    ...
    watch(selectedStudent, (val) => {
      ...
      form.value.student_id = val.id
      form.value.class = val.class || ''

      console.log('Selected student changed:', form.value)
    })
